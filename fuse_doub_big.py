...
def fuse(path,envname,param):
        ...
        for episode in range(num_episodes):
            ...
            for t in range(max_number_of_steps):
                ...
                # virtual_word[0][8] = -100
                # virtual_word[1][2] = -100
                # virtual_word[1][3] = -100
                # virtual_word[3][3] = -100
                # virtual_word[3][4] = -100
                # virtual_word[8][7] = -100
                # virtual_word[9][4] = -100
                # virtual_word[9][3] = -100
                ...
                #print(v_state)
                virtual_word[int(v_state/10)][int(v_state%10)] = -math.sqrt((9 - int(v_state/10))**2 + (9 - int(v_state%10))**2)
                # #print(virtual_word)
                print(virtual_word)
                flag = 0
                a_score = np.zeros([4])
                while flag<4:
                    ...
                    if key not in transfer:
                        ...
                        if virtual_word[int(next_state/10)][int(next_state%10)]==-100:
                            ...
                            action = (action+1)%4
                            get_action(v_state,action,next_state,virtual_word[int(next_state/10)][int(next_state%10)],0.5)
                        else:
                ...
                #如果没有出界且reward相差很大
                if observation!=-1 and abs(virtual_word[int(observation/10)][int(observation%10)] - reward) > 50:
                    print('训练数据')
                    label = 1
                    l_x = (int(observation%4)+1)*100+10
                    l_y = (int(observation/4)+1)*100+10
                    r_x = l_x+100
                    r_y = l_y+100
                    image.save(root_dir+'data/data_process/render_img/%d.jpg'%n)
                    ...
                    if n%7==0:#分配训练集和验证集
                        ...
                        list_file = open(root_dir+'data/data_process/train_val_data/kongzhi_val.txt','a')#a为追加写入，w为覆盖写入，r为只读
                        ...
                        list_file = open(root_dir+'data/data_process/train_val_data/kongzhi_train.txt','a')#a为追加写入，w为覆盖写入，r为只读
                        list_file.write(root_dir+'data/data_process/render_img/%d.jpg' % (n))
                        list_file.write(' '+'%d,%d,%d,%d,%d\n'%(l_x,l_y,r_x,r_y,label)) 
                        n=n+1
                ...
                if done:
                    ...
                    #         绘制平均分关于迭代次数的图像     
                    #------------------------------------------------
                    draw_file_1 = open(root_dir+'data/draw/1010/fuse_double_episode.txt','a')
                    draw_file_2 = open(root_dir+'data/draw/1010/fuse_double_reward.txt','a')
                    draw_file_1.write('%d\n'%episode)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, default='E:/gankong/yolov5-fuse-1.0/', help='root path')
    args = parser.parse_args()
    draw_file_1 = open(args.path+'data/draw/1010/fuse_double_episode.txt','a')
    draw_file_2 = open(args.path+'data/draw/1010/fuse_double_reward.txt','a')
